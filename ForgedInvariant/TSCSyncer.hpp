...
class TSCForger {
	...
	// Remove resetAdjust cuz we dont use it anymore
	
	void lockFreq();
	static void sync(void *);

	// wrapper hook function
	static void wrapXcpmUrgency(int urgency, uint64_t rtPeriod, uint64_t rtDeadline);
	static void wrapTracePoint(void *that, uint8_t point);
